...
class StarsDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if not hparams.get('noise_in_test', False) or hparams.get('noise_prob', 0.8) == 0.0:
            noise_added = False
        if noise_added:
        ...
        sample = {
            ...
            "ph": ph,
            "word": item.get('txt', item.get('word', '')),
            "ph2words": ph2words,
            "mel": spec,
        ...
        if hparams.get('use_soft_ph_bd', False) and (hparams.get('soft_ph_bd_func', None) not in ['none', None]):
            ...
            sample['ph_bd_soft'] = ph_bd_soft[:T]
        ...
        sample['note_num'] = torch.LongTensor(item.get('note_num', [1]))
        ...
        if hparams.get('use_soft_note_bd', False) and (hparams.get('soft_note_bd_func', None) not in ['none', None]):
            ...
            sample['note_bd_soft'] = note_bd_soft[:T]
