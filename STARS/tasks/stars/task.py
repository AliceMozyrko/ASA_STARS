...
class StarsTask(BaseTask):
    ...
    def add_ph_bd_loss(self, ph_bd_logits, ph_bd_soft, losses):
        if self.global_step >= hparams.get('ph_start', 0):
            if not hasattr(self, 'ph_bd_pos_weight'):
                self.ph_bd_pos_weight = torch.ones(hparams['max_frames']).to(ph_bd_logits.device)    # cache
            ...
            T = ph_bd_logits.shape[1]
            ph_bd_soft = ph_bd_soft[:, :T]
            print(f"ph_bd_logits shape: {ph_bd_logits.shape}")
            print(f"ph_bd_soft shape: {ph_bd_soft.shape}")
    ...
    def add_note_bd_loss(self, note_bd_logits, note_bd, losses):
        if not hasattr(self, 'note_bd_pos_weight'):
            self.note_bd_pos_weight = torch.ones(hparams['max_frames']).to(note_bd_logits.device)    # cache
            if hparams.get('label_pos_weight_decay', 0.0) > 0.0:
