"""
prepare_ukrainian_metadata.py

Конвертує wav + lab файли у metadata_processed.json для STARS.

Логіка:
  - AP / pɑu між фонемами = межі "слів" (синтагм)
  - ph2words — індекс синтагми для кожної фонеми
  - pitches — заповнюється нулями (RMVPE витягне F0 автоматично під час бінаризації)

Структура папок:
  wavs/   — .wav файли
  labs/   — .lab файли (формат: start end phoneme, секунди)

Використання:
  python prepare_ukrainian_metadata.py \
    --wav_dir wavs/ \
    --lab_dir labs/ \
    --output data/processed/ukrainian/metadata_processed.json \
    --speaker ukrainian_speaker

Після цього запусти бінаризацію:
  PYTHONPATH=. python data_gen/binarize.py --config configs/stars_ukrainian.yaml
"""
...
def parse_lab(lab_path: str) -> list[tuple[float, float, str]]:
    ...
    Читає .lab файл.
    Підтримує формати:
      - "0.0  0.06  pɑu"  (секунди, будь-який роздільник)
    """
    entries = []
...
def lab_to_item(
    ...
    """
    Конвертує список (start, end, phoneme) у запис metadata_processed.json.

    Поля:
      item_name   — назва без розширення
      wav_fn      — шлях до wav
      ph          — список фонем
      ph_durs     — тривалості фонем у секундах
      ph2words    — індекс синтагми для кожної фонеми (починаючи з 1)
      word        — список синтагм як рядків (об'єднані фонеми)
      word_durs   — тривалості синтагм у секундах
      pitches     — [0, 0, ...] — placeholder, RMVPE перезапише
      singer      — ім'я спікера
      lang        — мова
    """
    ph_list = []
    ...
    # Silence токени включаємо у поточну синтагму (вони потрібні моделі).
    word_groups = []      # list of list of (start, end, ph)
    current_group = []
    ...
    for start, end, ph in lab_entries:
        if ph in SILENCE_TOKENS:
            if current_group:
                # пауза після фонем = кінець синтагми, додаємо паузу в кінець
                current_group.append((start, end, ph))
                word_groups.append(current_group)
                current_group = []
            else:
                # пауза на початку або між паузами — чекаємо фонему
                pending_silences.append((start, end, ph))
        else:
            if not current_group:
                # нова синтагма — додаємо накопичені паузи на початок
    ...
    for word_idx, group in enumerate(word_groups):
        ...
        # Назва синтагми — об'єднані фонеми через "_"
        ...
        for start, end, ph in group:
            ...
            ph2words.append(word_idx + 1)  # 1-based, як mel2ph у STARS

    # Placeholder для pitch — RMVPE витягне автоматично під час бінаризації
